...
public class CommentDseDaoQueryProvider implements DseSchema {
    ...
    public CommentDseDaoQueryProvider(MapperContext context,
        ...
        this.psInsertCommentUser  = dseSession.prepare(entityHelperCommentsByUser.insert().asCql());
        this.psDeleteCommentUser  = dseSession.prepare(entityHelperCommentsByUser.deleteByPrimaryKey().asCql());
        this.psInsertCommentVideo = dseSession.prepare(entityHelperCommentsByVideo.insert().asCql());
        this.psDeleteCommentVideo = dseSession.prepare(entityHelperCommentsByVideo.deleteByPrimaryKey().asCql());
        
        psFindCommentsByUser = dseSession.prepare(
                ...
                .build());
        psFindCommentsByUserPageable = dseSession.prepare(
                selectFrom(TABLENAME_COMMENTS_BY_USER)
                .column(COMMENTS_COLUMN_USERID).column(COMMENTS_COLUMN_COMMENTID)
                .column(COMMENTS_COLUMN_VIDEOID).column(COMMENTS_COLUMN_COMMENT)
                .function("toTimestamp", Selector.column(COMMENTS_COLUMN_COMMENTID)).as("comment_timestamp")
                .where(column(COMMENTS_COLUMN_USERID).isEqualTo(bindMarker(COMMENTS_COLUMN_USERID)),
        ...
        psFindCommentsByVideo = dseSession.prepare(
                ...
                .build());
        psFindCommentsByVideoPageable = dseSession.prepare(
                selectFrom(TABLENAME_COMMENTS_BY_VIDEO)
                .column(COMMENTS_COLUMN_USERID).column(COMMENTS_COLUMN_USERID)
                .column(COMMENTS_COLUMN_VIDEOID).column(COMMENTS_COLUMN_COMMENT)
                .column(COMMENTS_COLUMN_COMMENTID)
    ...
    public ResultListPage<CommentByVideoEntity> findCommentsByVideoId(final QueryCommentByVideo query) {
        return new ResultListPage<>(dseSession.execute(buildStatementVideoComments(query))
    ...
    public CompletionStage<ResultListPage<CommentByVideoEntity>> findCommentsByVideoIdAsync(final QueryCommentByVideo query) {
        return dseSession.executeAsync(buildStatementVideoComments(query))
    ...
    public ResultListPage<CommentByUserEntity> findCommentsByUserId(final QueryCommentByUser query) {
        return new ResultListPage<>(dseSession.execute(buildStatementUserComments(query))
    ...
    public CompletionStage<ResultListPage<CommentByUserEntity>> findCommentsByUserIdAsync(final QueryCommentByUser query) {
        return dseSession.executeAsync(buildStatementUserComments(query))
    ...
    protected static <T> BoundStatement bind(PreparedStatement preparedStatement, T entity, EntityHelper<T> entityHelper) {
